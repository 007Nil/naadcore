...
|---|---|---|---|---|---|---|---|
| `soundfont_path` | string (filesystem path) | compiled-in `HARMONIUM_SOUNDFONT_PATH` (in-repo `plugins/harmonium/soundfonts/harmonium_v3.sf2`; CMake bakes the ABSOLUTE path in, so `get_config` echoes e.g. `<repo>/plugins/harmonium/soundfonts/harmonium_v3.sf2`) | any string (not validated) | **init-only** — used by `sfload` at `init()`; setting it after init does NOT reload | stored string | never rejected; a bad path fails `init()` with `PLUGIN_ERROR` ("Failed to load SoundFont") | `fluid_synth_sfload` |
...
| `coupler` | enum | `off` | `on` \| `off` (case-sensitive, exact) | **live, new presses only** (user spec, 2026-09-19) — a mid-hold toggle does NOT retro-add/remove the octave voice on already-held notes; notes pressed while on keep their octave voice until their own NoteOff, which releases every voice started at press time (`HeldNote.layers`); bellows reference untouched | `on` / `off` | `PLUGIN_INVALID_PARAM` (`1`, `On`, `"on "`, empty); state unchanged | `fluid_synth_noteon/noteoff` on internal **channel 15** (note+12, silently skipped above note 115) at **PARITY with the main voice** (user spec, 2026-09-20): CC 7 = 100 — FluidSynth's default channel volume, the same value the untouched main channels sit at — started at the note's own played velocity, EXACTLY +12 semitones (the former +3¢ detune was removed: the octave must be exact; the old CC 7 = 60 put it ~9 dB under the main voice). Pitch bend + CC 11 mirrored to it, CC 7 never |
...
- 2026-09-19 (coupler fix): `coupler` semantics aligned to the user spec —
  new presses only, no retroactive mid-hold application; the +3¢ tuning
  now uses absolute cents. `sub_octave` unchanged.
- 2026-09-20 (coupler parity): the octave voice is now at PARITY with the
  main voice and EXACTLY +12 semitones per the user spec — CC 7 raised
  60 → 100 (FluidSynth's default channel volume; the old value put the
  octave ~9 dB under the main, too subtle to hear as octave doubling),
  the +3¢ detune and all MIDI-tuning code removed (the 2026-09-19
  absolute-cents fix is thereby moot). Acoustic gate extended: the
  octave must measure within ±2 dB of the main voice for notes 48/60/72
