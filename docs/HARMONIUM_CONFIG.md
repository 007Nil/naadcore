...
| `coupler` | enum | `off` | `on` \| `off` (case-sensitive, exact) | **live, new presses only** (user spec, 2026-09-19) — a mid-hold toggle does NOT retro-add/remove the octave voice on already-held notes; notes pressed while on keep their octave voice until their own NoteOff, which releases every voice started at press time (`HeldNote.layers`); bellows reference untouched | `on` / `off` | `PLUGIN_INVALID_PARAM` (`1`, `On`, `"on "`, empty); state unchanged | `fluid_synth_noteon/noteoff` on internal **channel 15** (note+12, silently skipped above note 115), fixed gain CC 7 = 60, +3¢ tuning via `fluid_synth_activate_key_tuning` with **absolute cents (100·key + 3)** — the API takes each key's absolute pitch, not a detune offset (bug fixed 2026-09-19: constant +3 made every coupler voice ~8 Hz subsonic); pitch bend + CC 11 mirrored to it, CC 7 never |
